import {signInAnonymously, updateProfile, User as FirebaseUser} from "firebase/auth";
import {
    addDoc,
    collection,
    doc,
...
export function onRoomChanged(roomId: string, callback: (room: RoomCreated) => void) {
    const docRef = doc(db, "rooms", roomId);
    return onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
            const data = RoomSchema.parse(docSnap.data());
